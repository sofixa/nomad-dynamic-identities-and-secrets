...
job "gcs-job" {
  ...
  group "gcs-group" {
    ...
    task "gcs-task" {
      ...
      config {
        command        = "/bin/sh"
        args           = ["-c", "echo 'running; check stderr' && gcloud auth login --cred-file=/local/cred.json && gcloud storage cp /local/test.txt gs://${NOMAD_META_bucket} && sleep 5000000000"]
...
Some secret:  {{ with nomadVar "nomad/jobs/gcs-job" }}{{ .secret }}{{ end }}
EOF
      }
